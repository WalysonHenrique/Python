...
# Inicializa a expressão como uma string vazia
expression = ""
...
# Função para limpar a expressão e o campo de exibição
def clear():
    global expression
    expression = ""
    equation.set("")
...
if __name__ == "__main__":
    ...
    # Cria um campo de texto para exibir a expressão
    equation = StringVar()
    expression_field = Entry(gui, textvariable=equation)
    expression_field.grid(columnspan=4, ipadx=70)
    

    # Cria os botões para os dígitos e operações
    ...
    clear = Button(gui, text='Limpar', fg='black', bg='red', command=clear, height=1, width=7)
